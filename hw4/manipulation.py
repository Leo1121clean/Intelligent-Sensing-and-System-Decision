...
def load_kpts_from_depth(intrinsic : list or np.ndarray, keypoints : list or np.ndarray, depth_path : str):
    ...
    return kpts_3d # many key points
...
# first have:two 3D points from 2 RGBD, then 3D to 2D, and use 2D point to do SVD.
# SVD can calculate transformation matrix between 2 camera frames(3D)
...
def robot_dense_action(robot, obj_id : int, to_pose : list, grasp : bool=False, resolution : float=0.001, gripper2obj = np.identity(4)):
    ...
    for i in range(len(waypoints)): # waypoint:gripper
...
def main(args):
    ...
    f.close()
    template_gripper_transform = np.asarray(template_grasping['obj2gripper']) # gripper in obj frame, from obj to gripper
    template_obj_transform = np.linalg.inv(template_gripper_transform) # obj in gripper frame, from gripper to obj
    ...
    while repeat:
        ...
        # grasping pose
        gripper_grasping_trans = template2init @ template_gripper_transform # template2init:obj, and obj to gripper transform
